...
use crate::packet::{parse_dns_query, parse_sni, parse_v4};
...
impl Engine {
    ...
    pub fn handle_packet(&mut self, data: &mut [u8], app_id: Option<&str>) -> bool {
        ...
        if let Some(info) = parse_v4(data) {
            ...
            if self.stealth_mode {
                self.booster.apply_privacy_shield(data);
            }
            ...
            if self.packet_sizes.len() > 20 {
                self.packet_sizes.remove(0);
            }
            if self.ports.len() > 20 {
                self.ports.remove(0);
            }

            if let Some((risk, pattern)) =
                analyze_behavior(&self.packet_sizes, &self.inter_times, &self.ports)
            {
                if risk > 80 {
            ...
            if info.protocol == 17 {
                ...
                domain = parse_sni(&info.payload);
            }

            let allowed = self.firewall.analyze_packet(
                data,
                &info.dst_ip,
                info.dst_port,
                info.protocol,
    ...
    pub fn set_ai_active(&mut self, enabled: bool) {
        self.firewall.set_ai_active(enabled);
    }
    pub fn set_neural_shield(&mut self, enabled: bool) {
        self.neural_shield = enabled;
    }
    pub fn set_shaping_mode(&mut self, mode: i32) {
        self.shaping_mode = mode;
    }
    pub fn set_buffer_size(&mut self, size: u32) {
        self.buffer_size = size;
    }
    pub fn set_battery_safeguard(&mut self, enabled: bool) {
        self.battery_safeguard = enabled;
    }
    pub fn set_obfuscation_active(&mut self, enabled: bool) {
        self.booster.obfuscation_active = enabled;
    }

    pub fn set_booster_active(&mut self, enabled: bool) {
        self.booster.booster_active = enabled;
    }
    pub fn set_multipath_active(&mut self, enabled: bool) {
        self.booster.multipath_active = enabled;
    }

    pub fn get_app_rule(&self, app_id: &str) -> u8 {
        self.firewall.get_app_state(app_id)
    }
    ...
    pub fn add_whitelist(&mut self, val: &str, is_domain: bool) {
        if is_domain {
            self.firewall.whitelist_domain(val);
        } else {
            self.firewall.whitelist_ip(val);
        }
    }
    pub fn remove_whitelist(&mut self, val: &str, is_domain: bool) {
        if is_domain {
            self.firewall.unwhitelist_domain(val);
        } else {
            self.firewall.unwhitelist_ip(val);
        }
    }
    pub fn add_blacklist(&mut self, val: &str, is_domain: bool) {
        if is_domain {
            self.firewall.block_domain(val);
        } else {
            self.firewall.block_ip(val);
        }
    }
    pub fn remove_blacklist(&mut self, val: &str, is_domain: bool) {
        if is_domain {
            self.firewall.unblock_domain(val);
        } else {
            self.firewall.unblock_ip(val);
        }
    }
    pub fn add_geo_block(&mut self, country: String) {
        self.firewall.add_geo_block(country);
    }
    pub fn remove_geo_block(&mut self, country: String) {
        self.firewall.remove_geo_block(country);
    }
    pub fn add_port_block(&mut self, port: u16) {
        self.firewall.add_port_block(port);
    }
    pub fn remove_port_block(&mut self, port: u16) {
        self.firewall.remove_port_block(port);
    }
    pub fn update_ai_risk(&mut self, target: String, risk: u8) {
        self.firewall.update_ai_risk(target, risk);
    }
    ...
    pub fn set_honeypot_mode(&mut self, enabled: bool) {
        self.advanced.honeypot_active = enabled;
    }
    ...
    pub fn apply_dpi_script(&mut self, pattern: String, action: String) {
        self.advanced.add_dpi_script(pattern, action);
    }
}
