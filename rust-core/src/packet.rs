...
pub fn parse_sni(payload: &[u8]) -> Option<String> {
    ...
    if tls_data.len() < 6 || tls_data[5] != 0x01 {
        return None;
    }
    let mut i = 43;
    if tls_data.len() < i + 10 {
        return None;
    }
    while i < tls_data.len().saturating_sub(5) {
        if tls_data[i] == 0x00 && tls_data[i + 1] == 0x00 {
            ...
            if tls_data.len() < i + 4 + ext_len as usize {
                return None;
            }
            ...
            if sni_data.len() > 5 && sni_data[2] == 0x00 {
                ...
                if sni_data.len() >= 5 + name_len as usize {
                    return Some(
                        String::from_utf8_lossy(&sni_data[5..5 + name_len as usize]).to_string(),
                    );
                }
            }
        }
